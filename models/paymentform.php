 ...
 * Confmgt model.
 */
class ConfmgtModelPaymentForm extends JModelForm
{
	...
	protected function populateState()
	{
        ...
        if (JFactory::getApplication()->input->get('layout') == 'edit') {
            $id = JFactory::getApplication()->getUserState('com_confmgt.edit.payment.id');
        } else {
            $id = JFactory::getApplication()->input->get('id');
            JFactory::getApplication()->setUserState('com_confmgt.edit.payment.id', $id);
        }
	...
	public function getTable($type = 'Payment', $prefix = 'ConfmgtTable', $config = array())
	{   
	...
	{
		// Get the id.
		$id = (!empty($id)) ? $id : (int)$this->getState('payment.id');
	...
	{
		// Get the form.
		$form = $this->loadForm('com_confmgt.payment', 'paymentform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$data = JFactory::getApplication()->getUserState('com_confmgt.edit.payment.data', array());
        if (empty($data)) {
            $data = $this->getData();
        }
	...
	{
		$id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('payment.id');
