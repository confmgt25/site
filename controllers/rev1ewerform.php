...
class ConfmgtControllerRev1ewerForm extends ConfmgtController
{
	...
	public function save()
	{
		...
		if (!$edit) // not in the edit mode, hence a new reviewer
		{
			...
			if ($userexists > 0)
			{
				...
				if (!$reviewerexists == false) {
					...
					}elseif ($reviewerexists->agreed ==1) { //reviewer agreed and already in the system
				...
				}else{ //user exists but not as a reviewr in the reviewer table. We have to create a new reviewer and assign the existing userid.
					...
					$placeholders = array('NAME'=>$name,'SITE'=>$sitename, 'AGREE_LINK' =>$agreelink, 'DENY_LINK' =>$denylink, 'THEME_LEADER' =>$theme_leader);
		...
		}else{ //reviewer record in the edit mode, hence no need to generate the emails, etc
