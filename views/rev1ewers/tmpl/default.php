...
?>
      ...
      <?php //TO-DO change the default ACL to confmgt ACL 
						...
						?>
      <tr>
        <td width="2%"><?php echo $item->ordering; ?></td>
        <td width="33%"><a href="<?php echo JRoute::_('index.php?option=com_confmgt&view=rev1ewer&id=' . (int)$item->id); ?>"><?php echo $item->title.' '.$item->firstname.' '.$item->surname; ?></a></td>
        <td width="15%"><?php 
							if ($item->agreed ==1) {
							...
							?></td>
        <td width="15%"><?php echo $item->papers; ?></td>
        <td width="30%"><?php	
									//TO-DO Change to confmgt ACL
									//if(JFactory::getUser()->rev1ewerise('core.delete','com_confmgt')):
									?>
          <form id="form-rev1ewer-delete-<?php echo $item->id; ?>" style="display:inline" action="<?php echo JRoute::_('index.php?option=com_confmgt&task=rev1ewer.remove'); ?>" method="post" class="form-validate" enctype="multipart/form-data">
          ...
          <form id="form-rev1ewer-edit-<?php echo $item->id; ?>" style="display:inline" action="<?php echo JRoute::_('index.php?option=com_confmgt&task=rev1ewer.edit&id='.$item->id); ?>" method="post" class="form-validate" enctype="multipart/form-data" >
